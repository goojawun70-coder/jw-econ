...
- **종목코드 먼저 확인**: 가격 검색 전에 "{회사명} 종목코드"로 정확한 코드를 확인하고, 시가총액·업종으로 교차 확인 후 가격을 검색 (지주회사·계열사 혼동 주의 — 예: 아모레퍼시픽 090430 vs 아모레퍼시픽홀딩스 002790). 기억이나 추정에 의존해 코드를 바로 쓰지 말 것 — 2026-09-11 발행 중 HD현대중공업을 009540(실제로는 HD한국조선해양)으로 잘못 짚었다가 기업명 대조 과정에서 발견해 329180으로 정정한 사고, 2026-09-12 발행 중 샌즈랩을 304100(실제로는 솔트룩스)으로 잘못 짚었다가 411080으로 정정한 사고가 있었다. 페이지를 가져온 후 반드시 "종목명"이 요청한 회사명과 정확히 일치하는지 확인할 것.
...
**2026-09-13~15 확인 — 네이버금융 종목 페이지의 PER/PBR/ROE/부채비율/목표주가/투자의견/당일 외국인·기관 순매수량 필드가 정적 스크래핑에 자주 실패**: `finance.naver.com/item/main.naver?code=...`(PC)가 빈 JS 셸만 반환하는 경우가 잦아져 `m.stock.naver.com/domestic/stock/{code}/total`(모바일)로 대체하는 일이 많아졌는데, 이 모바일 페이지조차 PER·PBR·ROE·부채비율·목표주가·투자의견·당일 순매수량은 별도 AJAX 탭에서 로딩되어 `apify--rag-web-browser`의 정적 스크래핑으로는 "-"만 잡히고 값을 가져오지 못하는 경우가 3거래일 연속(9/13 HMM·유한양행, 9/14 삼성전자·KT&G, 9/15 리노공업·빙그레) 반복 확인됐다. 가격·52주 범위·시가총액·외국인소진율은 대체로 확보되나, 나머지 필드는 확보 실패 시 "확보 못함"이라고 정직하게 표기하고 수치를 지어내지 말 것. **2026-09-16 갱신 — FnGuide 대체 소스로 확보 성공**: `comp.wisereport.co.kr/company/c1010001.aspx?cmp_cd={종목코드}` (FnGuide 컨센서스 페이지)를 `apify--rag-web-browser`로 스크래핑했더니 PER·PBR·추정치·목표주가 컨센서스(증권사별 목표가 포함)가 정상적으로 확보됐다(하나금융지주·오리온 사례). 앞으로 네이버금융 모바일 페이지에서 PER/PBR/목표주가가 "-"로 나오면, 바로 포기하지 말고 이 FnGuide URL을 1차 대체 소스로 먼저 시도할 것. 다만 FnGuide 페이지의 "기준" 날짜 표기가 실제 최신 종가와 하루 정도 어긋나 보일 때가 있으니, 종가 자체는 네이버금융의 "기준가"/오늘 날짜 값을 우선하고 PER·PBR·목표주가 등 컨센서스 지표만 FnGuide 값으로 보완할 것.

**2026-09-16 확인 — 종목코드 재확인 사고 추가 사례**: 오리온(제과, 271560)을 검색하다 001800(오리온홀딩스, 지주사)을 잘못 짚은 사고가 있었다. "종목코드 먼저 확인" 원칙을 다시 한번 강조 — 특히 지주사 개편이 있었던 대기업 계열은 종목명이 비슷해 혼동하기 쉽다.

**2026-09-23 확인 — 원익IPS 종목코드 오기 사고**: 원익IPS의 종목코드를 042420으로 잘못 기억해 지시했으나, 실제 042420은 네오위즈홀딩스였다. 원익IPS의 정확한 종목코드는 **240810**이다(스크래핑 후 종목명 대조 과정에서 발견해 정정). 기억에 의존해 종목코드를 바로 쓰지 말라는 원칙이 재확인된 사례 — 원익그룹 계열사(원익IPS·원익머티리얼즈·원익QnC 등)도 이름이 비슷해 혼동 위험이 있다.

**2026-09-24 확인 — 동서 종목코드 오기 사고 + 재발 방지 절차 강화**: 동서(주)의 종목코드를 049770으로 잘못 기억해 지시했으나, 이 코드는 네이버금융에 존재하지 않는 코드였다(빈 리다이렉트만 발생). 데이터 수집 에이전트가 자체적으로 회사명 대조 중 이상을 감지하고 "{회사명} 종목코드"를 웹검색으로 재확인해 정확한 코드 **026960**을 찾아 정정했다. 이번까지 국내 종목코드 오기 사고가 총 5건(HD현대중공업, 샌즈랩, 오리온, 원익IPS, 동서) 누적됐다 — 기억에 의존한 종목코드 지시가 계속 반복되는 근본 원인이므로, **앞으로는 국내 종목을 심층분석 종목으로 선정할 때, 데이터 수집 에이전트에게 넘기기 "전에" 먼저 WebSearch로 "{회사명} 종목코드"를 검색해 코드를 확정한 뒤 지시할 것** (또는 데이터 수집 에이전트 프롬프트 자체에 "네이버 페이지가 빈 리다이렉트를 반환하거나 종목명이 불일치하면 즉시 WebSearch로 정확한 코드를 재확인하라"는 지시를 항상 포함할 것). 이 이중 확인 절차를 생략하지 말 것.

**2026-09-10 확인 — 네이버금융 리서치 게시판 자체가 개편으로 작동 중단**: `finance.naver.com/research/company_list.naver`(날짜 필터 유무 무관)가 더 이상 리포트 목록을 보여주지 않고 신규 `stock.naver.com/research/company` 페이지로 리다이렉트되는데, 이 페이지는 "국내종목/발행사/발행일자 검색"만 있는 빈 폼이라 서버사이드 렌더링으로도 목록이 나오지 않는다(로그인 요구). 대체 경로가 확인되기 전까지는 이 섹션 전체를 건너뛰거나, 각 종목의 `item/main.naver` 페이지에 함께 표시되는 에프앤가이드 컨센서스 목표주가·투자의견(날짜 태그 포함)으로 대체할 것. WebSearch로 찾은 리포트는 날짜가 오늘/어제와 맞는지 반드시 확인(오래된 기사가 "최신"인 것처럼 섞여 나옴 — 2026-09-10 시도 시 5월·7월자 리포트만 검색됨).
